...
Response SocketIO::getResponse() {
    Response response;
    readn(*this->s, reinterpret_cast<char *>(&response), sizeof response);
    return response;
}
...
std::string SocketIO::getString() {
    size_t size = 0;
    auto nr = readn(*this->s, reinterpret_cast<char *>(size), sizeof size);
    if (nr <= 0 || nr > BUFFER_SIZE) {
        std::runtime_error("Receive failed");
    }
    readn(*this->s, buffer, nr);
    buffer[nr] = '\0';
